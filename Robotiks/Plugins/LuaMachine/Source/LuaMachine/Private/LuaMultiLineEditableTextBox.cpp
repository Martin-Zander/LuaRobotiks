...
TSharedRef<SWidget> ULuaMultiLineEditableTextBox::RebuildWidget()
{
	...
	if (LuaState)
	{
		TArray<FString> HighlightingTokens;

		ULuaState* LuaStateObj = NewObject<ULuaState>(this, LuaState);
		LuaStateObj->Table.GetKeys(HighlightingTokens);
		for (FString& Token : HighlightingTokens)
		{
			Style.CustomTextColorMapping.Add(Token, LuaStateNamesColor);
		}
	}
